...
export const nameStyleCommand = {
  name: "namestyle",
  description: "Customize the bot's Discord display name style in this server",
};
...
export function createNameStylePanel(state) {
    ...
    .addOptions(
      FONT_OPTIONS.map(([label, value]) =>
        new StringSelectMenuOptionBuilder()
          .setLabel(label)
          .setValue(value)
          .setDefault(value === String(state.font))
      )
    );
    ...
    .addOptions(
      EFFECT_OPTIONS.map(([label, value]) =>
        new StringSelectMenuOptionBuilder()
          .setLabel(label)
          .setValue(value)
          .setDefault(value === String(state.effect))
      )
    );
  ...
  return new ContainerBuilder()
    ...
    .addActionRowComponents(
      new ActionRowBuilder().addComponents(fontSelect)
    )
    .addActionRowComponents(
      new ActionRowBuilder().addComponents(effectSelect)
    )
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
...
export const nameStyleFlags = MessageFlags.IsComponentsV2;
